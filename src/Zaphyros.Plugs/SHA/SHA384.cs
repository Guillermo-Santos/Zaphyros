...
{
    public static class CustomSHA384
    {
        ...
        private static ulong Sum0(ulong x) => RotateRight(x, 28) ^ RotateRight(x, 34) ^ RotateRight(x, 39);

        private static ulong Sum1(ulong x) => RotateRight(x, 14) ^ RotateRight(x, 18) ^ RotateRight(x, 41);

        private static ulong Sigma0(ulong x) => RotateRight(x, 1) ^ RotateRight(x, 8) ^ x >> 7;

        private static ulong Sigma1(ulong x) => RotateRight(x, 19) ^ RotateRight(x, 61) ^ x >> 6;

        private static ulong RotateRight(ulong value, int count) => value >> count | value << 64 - count;

        private static ulong ConvertBytesToUInt64(byte[] data, int startIndex)
        {
            ulong result = 0;
            for (int i = 0; i < 8; i++)
            {
                result <<= 8;
                if (startIndex + i < data.Length) // Add this check to prevent index out of range
                {
